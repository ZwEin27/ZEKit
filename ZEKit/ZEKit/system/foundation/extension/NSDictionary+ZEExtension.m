...
@implementation NSDictionary(ZEExtension)
...
- (id)objectForClass:(Class)clazz
{
	...
	for ( Class clazzType = clazz; clazzType != [NSObject class]; )
	{
		...
		for ( NSUInteger i = 0; i < propertyCount; i++ )
		{
			...
			NSUInteger		type = [ZETypeEncoding typeOf:attr];
			...
			if ( tempValue )
			{
				...
				{
					NSString * className = [ZETypeEncoding classNameOfAttribute:attr];
...
@implementation NSMutableDictionary(ZEExtension)
